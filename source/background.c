...
/*   if (pba->has_cdm == _TRUE_) {
  ...
  if (pba->has_cdm == _TRUE_) {
    ...
    double w_cdm = pba->w0_cdm + pba->wa_cdm * (1. - a);
    pvecback[pba->index_bg_w_cdm] = w_cdm;
  
    /* Calculate density evolution following the formula:
      rho_dm/rho_dm,0 = exp(-3*w_dm,a*(1-a)) * a^(-3(1+w_dm,0+w_dm,a)) */
    pvecback[pba->index_bg_rho_cdm] = pba->Omega0_cdm * pow(pba->H0,2) * 
                                      exp(-3. * pba->wa_cdm * (1.-a)) * 
                                      pow(a, -3. * (1. + pba->w0_cdm + pba->wa_cdm));
  
    /* Add CDM contribution to total density */
    rho_tot += pvecback[pba->index_bg_rho_cdm];
  
    /* Calculate and add CDM pressure: p = w * rho */
    pvecback[pba->index_bg_p_cdm] = w_cdm * pvecback[pba->index_bg_rho_cdm];
    p_tot += pvecback[pba->index_bg_p_cdm];
  
    /* For dp_dloga calculation */
    double dw_over_da = -pba->wa_cdm;
    dp_dloga += (a * dw_over_da - 3. * (1. + w_cdm) * w_cdm) * pvecback[pba->index_bg_rho_cdm];

    rho_m += pvecback[pba->index_bg_rho_cdm];
  
    /* Add to matter or radiation density */
/*     if (fabs(w_cdm) < 1e-6) {
      rho_m += pvecback[pba->index_bg_rho_cdm];
    } else {
      rho_m += pvecback[pba->index_bg_rho_cdm] * (1. - 3. * w_cdm) / (1. + 3. * w_cdm);
      rho_r += 3. * w_cdm * pvecback[pba->index_bg_rho_cdm] / (1. + 3. * w_cdm);
    } */
  }
...
int background_w_cdm(
  ...
  *w_cdm = pba->w0_cdm + pba->wa_cdm * (1. - a);
