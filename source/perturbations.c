...
int perturbations_vector_init(
  ...
  if (_scalars_) {
    ...
    class_define_index(ppv->index_pt_theta_cdm, pba->has_cdm && ((ppt->gauge == newtonian) || (pba->w0_cdm != 0.0) || (pba->wa_cdm != 0.0)), index_pt, 1); /* Hanyu cdm velocity */
...
int perturbations_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    if (pba->has_cdm == _TRUE_) {
      ...
      w_prime_cdm = -a * a_prime_over_a * pba->wa_cdm;
      ...
      if (ppt->gauge == synchronous) {
        if ((pba->w0_cdm != 0.0) || (pba->wa_cdm != 0.0)) {
